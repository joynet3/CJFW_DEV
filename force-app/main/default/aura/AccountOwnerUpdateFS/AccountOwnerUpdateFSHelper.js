({
    ...
    doInit : function(component, event, helper) {
        ...
        component.set("v.listColumn", [
            {'label' : '고객사명', 'fieldName' : 'name', 'type': 'text', sortable: true},
            {'label' : '담당자명', 'fieldName' : 'ownerName', 'type': 'text', sortable: true},
            {'label' : '사원번호', 'fieldName' : 'ownerEmployeeNumber', 'type': 'text', sortable: true},
            {'label' : '고객경로', 'fieldName' : 'BusinessPathLv1', 'type': 'text', sortable: true},
            {'label' : '운영형태', 'fieldName' : 'TypeOfOperation', 'type': 'text', sortable: true},
            {'label' : '계약만료일', 'fieldName' : 'ContractExpireDate', 'type': 'text', sortable: true},
        ...
        component.set("v.listColumnChange", [
            ...
            {'label' : '변경 전 담당 FS', 'fieldName' : 'ownerName', 'type': 'text', initialWidth: 150, sortable: true},
            {'label' : '변경 전 담당 FS 사원번호', 'fieldName' : 'ownerEmployeeNumber', 'type': 'text', initialWidth: 205, sortable: true},
            {'label' : '변경 후 담당 FS', 'fieldName' : 'userName', 'type': 'text', initialWidth: 150},
            {'label' : '변경 후 담당 FS 사원번호', 'fieldName' : 'employeeNumber', 'type': 'text', initialWidth: 205},
    ...
    doChangeDate : function(component, event, helper) {
        var listSelected = component.get("v.listSelected");      
        let chunkSize = component.get("v.chunkSize");   
        helper.doChunk(component, event, helper, 'chgDt', listSelected, 0, (chunkSize < listSelected.length ? chunkSize : listSelected.length)); 
    },
    // Null , Undefined , '' 체크
    ...
    doChunk : function(component, event, helper, targetInfo, listTarget, startIdx, endIdx) {
        ...
        else if( targetInfo == 'chgDt'){
            action = component.get("c.doChangeDate");
            var selectedDate = component.get("v.selectedDate");    
            console.log('selectedDate >>'+selectedDate);
            console.log('JSON.stringify(listChunk) >>'+JSON.stringify(listChunk));
            action.setParams({
            ...
            if(state === "SUCCESS") {
                ...
                if(listTargetLength > endIdx) {
                    ...
                    if( targetInfo == 'chgOwner'){
                        ...
                        helper.showToast('success', '계약만료일자가 변경되었습니다.');  
                    }                    
                    setTimeout(function(){
